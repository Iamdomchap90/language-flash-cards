...
const updateThirtyDayActivity = (activityUser) => {
  ...
  thirtyDayActivityKeys.forEach((dateStr) => {
    if (dateStr <= thirtyDaysAgoStr) {
      delete activityUser.Russian.thirtyDayActivity.delete(dateStr);
    }
  });

  // Fill all dates between last date and today with 0.
  if (mostRecentDay < today) {
    const daysDifference = (today - mostRecentDay) / millisecsInDay;
  ...
  if (thirtyDayActivityKeys.includes(todayStr)) {
    let originalVal = activityUser.Russian.thirtyDayActivity.get(todayStr);
